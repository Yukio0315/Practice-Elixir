defmodule MusicDB.Repo.Migrations.AddCompositionArtistsTable do
  ...
  def up do
    ...
    create(index("compositions_artists", [:composition_id, :artist_id]))
    ...
    |> Enum.each(fn row ->
      Repo.insert_all("compositions_artists", [
        [compositions_id: row.id, artist_id: row.artist_id, role: "composer"]
      ])
    end)
  ...
  def down do
    ...
    |> Repo.all()
    |> Enum.each(fn row ->
      Repo.update_all(from(c in "compositions", where: c.id == ^row.compositions_id),
        set: [artist_id: row.artist_id]
      )
    end)

    drop(table("composition_artists"))
  end
end
